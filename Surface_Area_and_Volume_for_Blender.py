# This module calculates the surface area and volume a user-defined
# region specified by collection of vertices on a mesh.
# Two new meshes are created as children of the orignal mesh object:
#
#   - submesh_surf:   a mesh of exactly the originally selected vertices,
#                     its surface area is exactly that which was selected
#
#   - submesh_solid:  a mesh of the originally selected vertices with
#                     all holes closed, to be able to calculate the
#                     correct volume of the solid (its surface area
#                     includes any added triangles)
#
# A tool to calculate lengths by selecting points to define a curve
# is included.
#
# All objects in the scene can be rescaled, and meshes can be remeshed.
#
# Data can be exported into a comma-separated .txt file that can be
# imported by Microsoft Excel.


#    Surface_Area_and_Volume_for_Blender.py (C) 2013,  Anne Jorstad, Biagio Nigro, Diego Marcos
#
...
class CreateCurve(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if len(obj.vertex_collection)>=2:
          ...
          #vert_vector.sort()    
          new_name = obj.string_name + "_curve"
          ...
          poly.constraints['Child Of'].target = obj
          bpy.ops.constraint.childof_set_inverse(constraint=poly.constraints['Child Of'].name, owner='OBJECT')
          #bpy.ops.object.mode_set(mode='EDIT')
...
class CreatePath(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if len(obj.vertex_collection)==2:
          ...
          new_name = obj.string_name + "_path"
          curve = MakePolyLine(new_name, "length", vert_vector, obj)
          bpy.ops.object.mode_set(mode='OBJECT')
          poly=curve
          poly.select=True             
...
# called by Create_Submesh and Creaptrte_Open_Submesh
...
class Create_Closed_Submesh(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # to undo transform from .parent operation (http://www.foro3d.com/archive/index.php/t-102869.html)
          ...
          bpy.context.scene.objects.active = new_obj2
          bpy.ops.object.constraint_add(type='CHILD_OF')
          new_obj2.constraints['Child Of'].target = obj_ptr
          bpy.ops.constraint.childof_set_inverse(constraint=new_obj2.constraints['Child Of'].name, owner='OBJECT')
     ...
     if len(bpy.context.selected_objects)!=0:
      ...
      for obj in objlist:
        ...
        if flag==0:   
          ...
          new_obj.constraints['Child Of'].target = obj_ptr
          bpy.ops.constraint.childof_set_inverse(constraint=new_obj.constraints['Child Of'].name, owner='OBJECT')
          new_obj.name=new_obj.parent.name+"_surf"  
          bpy.ops.object.mode_set(mode='EDIT')
          bpy.ops.object.mode_set(mode='OBJECT')
          bpy.ops.object.mode_set(mode='EDIT')
    ...
    def execute(self, context):
        ...
        for this_obj in bpy.data.objects:
            ...
            if this_parent is not None:
                ...
                bpy.ops.object.constraint_add(type='CHILD_OF')
                this_obj.constraints['Child Of'].target = this_parent
                bpy.ops.constraint.childof_set_inverse(constraint=this_obj.constraints['Child Of'].name, owner='OBJECT')
                bpy.ops.object.mode_set(mode='EDIT')
                bpy.ops.object.mode_set(mode='OBJECT')
                bpy.ops.object.select_all(action='DESELECT')
...
class FolderButton(bpy.types.Operator):
    bl_idname = "file.folder"
    ...
    def execute(self, context):
        ...
        bpy.context.scene.file_path=directory
        print("File Directory = "+bpy.context.scene.file_path)
        return {'FINISHED'}
...
class Write_Data(bpy.types.Operator):
    bl_idname = "mesh.write_data"
    bl_label = "write data to file"
    bl_options = {"REGISTER", "UNDO"}
    
    def execute(self, context):
        the_name = bpy.context.scene.file_path+bpy.context.scene.file_name + '.txt'
        print(the_name)
        f = open(the_name, 'w')
...
class Write_Selected_Data(bpy.types.Operator):
    ...
    bl_label = "write selected data to file"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        the_name = bpy.context.scene.file_path+bpy.context.scene.file_name + '_more.txt'
        f = open(the_name, 'w')
        f.write('Object Name,Parent,Surface Area,Volume,Number of Vertices,Length\n\n')
        for obj in to_write:
            write_obj_info(obj, f)
...
#    bl_region_type = "UI"
    def draw(self, context):
        ...
        if hasattr(obj, 'data'):
            ...
            layout.label("--Create SA and Volume Meshes--")
            ...
            col3.operator("mesh.create_both_meshes", text = "Surface Area and Volume")
            row = layout.row()
            row.operator("mesh.create_whole_surfaces", text='Create Whole Surfaces')
        
        


            layout.label("--Define Points to Calculate Curve Length or Shortest Path Length--")
            ...
            layout.operator("create.meas", text = "create curve from at least 2 points")
            
            layout.operator("create.path", text = "create shortest path from 2 points")
            #layout.label("--Remesh--")
            #layout.prop(scn, 'remesh_octree_depth')
            #layout.operator("mesh.remesh", text = "apply remesh")

        layout.label("--Adjust Scene Scale--")
        split = layout.row().split(percentage=0.5)
        colL = split.column()
        colR = split.column()
        ...
        # name the object
        ### would like to have default index be ind+1
        ...
        if scn.obj_type == "N/A":
            ...
            global_name = this_name
        layout.operator("mesh.update_name", text = "update name")

        layout.label("--Output Data--")
        row = layout.row()
        
        row.prop(scn, 'file_name')
        row.operator("file.folder", text='File Path')
        
        row = layout.row()
        layout.operator("mesh.write_data", text = "write full file")
        row = layout.row()
        layout.operator("mesh.write_selected_data", text = "write sub file")
        row = layout.row()
...
def register():
    ...
    bpy.utils.register_class(FolderButton)
    bpy.utils.register_class(Write_Data)
    ...
    #bpy.types.Scene.file_name = StringProperty(name = "filepath", default="/Blender_Numbers")
    bpy.types.Scene.file_path = StringProperty(name = "filepath", default="/")
    bpy.types.Scene.file_name = StringProperty(name = "filename", default="Blender_Numbers")
    ...
    obj_types_enum = [("N/A","N/A","",1),("axon","axon","",2),("dendrite","dendrite","",3),\
                      ("synapse","synapse","",4),("bouton","bouton","",5),("spine","spine","",6)]
    axon_types_enum = [("N/A","N/A","",1),("_E","excitatory","",2),("_I", "inhibitory","",3),("_U", "unknown","",4)]
    synapse_types_enum = [("N/A","N/A","",1),("_E","excitatory","",2),("_I", "inhibitory","",3),("_U", "unknown","",4)]
    bpy.types.Scene.obj_type = EnumProperty(name = 'obj type', default = "N/A", items = obj_types_enum)
    bpy.types.Scene.axon_type = EnumProperty(name = 'axon type', default = "N/A", items = axon_types_enum)
    ...
    bpy.types.Scene.synapse_s_ind = IntProperty(name = 'synapse index', default=0, min=0)
    bpy.types.Scene.synapse_a_ind = IntProperty(name = 'axon inxex', default=0, min=0)
    bpy.types.Scene.synapse_b_ind = IntProperty(name = 'buton index', default=0, min=0)
    bpy.types.Scene.synapse_type = EnumProperty(name = 'synapse type', default = "N/A", items = synapse_types_enum)
...
##    # force blender to cycle and display;  must be commented out for plugin
#    if bpy.ops.object.mode_set.poll():
#        bpy.ops.object.mode_set(mode='OBJECT')
#    bpy.ops.object.mode_set(mode='EDIT')
#    bpy.ops.object.mode_set(mode='OBJECT')
...
def unregister():
    ...
    bpy.utils.unregister_class(FolderButton)
    bpy.utils.unregister_class(Write_Data)
